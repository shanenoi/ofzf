(** Core fuzzy matcher and ranking functions for [ofzf]. *)
...
(** [match_candidate ~query ~candidate] performs a case-insensitive fuzzy
    ...
    It returns [None] when every query character cannot be found in order.
    It returns [Some result] with zero-based match positions and a numeric score
    when the candidate matches. A higher score is better. *)
...
(** [rank ~query candidates] filters and sorts candidates by descending score.
    Ties are resolved by the candidate text for deterministic output. *)
